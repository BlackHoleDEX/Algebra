...
import '@cryptoalgebra/alm-vault/contracts/interfaces/IAlmVault.sol';
...
abstract contract AlmPlugin is AlgebraBasePlugin, IAlmPlugin {
    ...
    // TODO: наверное стоит не принимать миллард параметров, а упаковать в типа InitializeData
    function initializeALM(address _vault, Thresholds memory _thresholds) public {
        ...
        bool _allowToken1 = IAlmVault(vault).allowToken1();
        ...
        address token0 = IAlmVault(_vault).token0();
        address token1 = IAlmVault(_vault).token1();
    ...
    function setDepositTokenUnusedThreshold(uint16 _depositTokenUnusedThreshold) external {
        _authorize();
        require(_depositTokenUnusedThreshold >= 100 && _depositTokenUnusedThreshold <= 10000, '_depositTokenUnusedThreshold must be 100 <= _depositTokenUnusedThreshold <= 10000');
    ...
    function _rebalance(TwapResult memory obtainTWAPsResult) internal {
        ...
        if (decideStatus != DecideStatus.PendingRebalanceNeeded) {
            if (decideStatus != DecideStatus.NoNeedWithPending) {
                if (decideStatus != DecideStatus.FailedToObtainTWAPOrExtremeVolatility) {
                    ...
                    // TODO: swapquantity ?
                    try IAlmVault(vault).rebalance(ranges.baseLower, ranges.baseUpper, ranges.limitLower, ranges.limitUpper, 1) {
                    ...
                    IAlmVault(vault).setDepositMax(0, 0);
        ...
        // IAlmVault(vault).rebalance(ranges.tick1, ranges.tick2, ....);
    }
    ...
    function _obtainTWAPs(
        ...
        if (_allowToken1) {
            ...
            (uint256 amount0, uint256 amount1) = IAlmVault(vault).getTotalAmounts();
    ...
    function _getRangesWithState(State newState, TwapResult memory twapResult) internal view returns (Ranges memory ranges) {
        // scope to prevent stack too deep
        {
            ...
            // State _state = state;
            bool _allowToken1 = allowToken1;
            int24 _tickSpacing = IAlgebraPool(pool).tickSpacing();
