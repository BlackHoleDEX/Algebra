...
async function main() {
    ...
    const BasePluginV5Factory = await hre.ethers.getContractFactory("BasePluginV5Factory");
    const dsFactory = await BasePluginV5Factory.deploy(deploysData.factory);
    ...
    const feeDiscountRegistryFactory = await hre.ethers.getContractFactory("FeeDiscountRegistry");
    const feeDiscountRegistry = await feeDiscountRegistryFactory.deploy(deploysData.factory);

    await feeDiscountRegistry.waitForDeployment()

    console.log("FeeDiscountRegistry to:", feeDiscountRegistry.target);
