...
import { algebraCoreFixture } from './shared/externalFixtures';

import { BasePluginV1Factory, AlgebraALMCustomPoolDeployer } from '../typechain';
...
describe('ALMPluginCustomDeployer', () => {
  ...
  let customPoolDeployer: AlgebraALMCustomPoolDeployer;
  ...
  beforeEach('deploy test volatilityOracle & custom deployer', async () => {
    ...
    factory = fix.factory;
    const customDeployerFactory = await ethers.getContractFactory('AlgebraALMCustomPoolDeployer');
    customPoolDeployer = (await customDeployerFactory.deploy(factory, fix.entryPoint)) as any as AlgebraALMCustomPoolDeployer;
