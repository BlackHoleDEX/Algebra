...
describe('AlgebraSecurityPlugin', () => {
  ...
  beforeEach('deploy test AlgebaraSecurityPlugin', async () => {
    ({ plugin, mockPool, pluginFactory, registry, mockFactory } = await loadFixture(feeDiscountPluginFixture));
  });
  ...
  describe('#FeeDiscountPlugin', () => {
    ...
    beforeEach('initialize pool', async () => {
      ...
      // set static fee
      await pluginFactory.setDefaultFeeConfiguration(config);
    });

    describe('default fee discount 0% ', async () => {
      it('works correct', async () => {
        expect(mockPool.mint(wallet.address, wallet.address, 0, 60, 100, '0x'))
  ...
  describe('AlgebarFeeDiscountPlugin external methods', () => {
    ...
    it('set registry contract works correct', async () => {
      ...
      await expect(plugin.setFeeDiscountRegistry(registry)).to.emit(plugin, 'SecurityRegistry');
